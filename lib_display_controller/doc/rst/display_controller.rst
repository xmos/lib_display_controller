...
The asynchronouse command buffering is handeled by the interfaces::
...
There is one other interface that connects to the application, this is the vertical synchronisation interface. The purpose of this interface is to allow the application to know when the frame is at the start of a new scan, i.e. line zero is about to be written.

As the display controller uses some of the SDRAM the memory address allocator is used to allocate an ammount of the SDRAM to the display contoller. See ... TODO to find out more about how to use the memory address allocator.

For example, the following code instantiates a display controller server
and connects and application to it (the SDRAM and LCD declarations has been shortened for simplicity)::
...
Note that the application, display controller, LCD server and SDRAM server 
must be on the same tile as the line buffers are transfered my moving pointers 
...
the display controller then whilst the server is proceessing the command the 
...
The display controller will start by displaying the image with handel 0. There is no need to initialise this frame as it will be set to all black (0x0). The application should then begin by writing to any other registered frames (image handel 1 and up).


Safety through the use of movable pointers
------------------------------------------
The API makes use of movable pointer to aid correct multi-threaded memory handeling. See ... TODO to know more about movable pointers.

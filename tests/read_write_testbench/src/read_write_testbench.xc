...
/*******************************************************************************
 * This program tests the SDRAM read and write through the display controller
*******************************************************************************/
...
void app(client interface app_to_cmd_buffer_i to_dc, client interface res_buf_to_app_i from_dc,
    ...
    unsigned buf_wr[LCD_ROW_WORDS], buf_rd[LCD_ROW_WORDS];
    unsigned * movable buf_wr_ptr = buf_wr;
    unsigned * movable buf_rd_ptr = buf_rd;

    for(unsigned i=0; i<LCD_ROW_WORDS; i++)
       buf_wr_ptr[i] = i;

    /* Write a line to SDRAM, read it back from SDRAM and check for correctness */
    display_controller_write(to_dc, move(buf_wr_ptr), 1, 0, LCD_ROW_WORDS, 0);
    display_controller_read(to_dc, move(buf_rd_ptr), 1, 0, LCD_ROW_WORDS, 0);

    unsigned r;
    {buf_wr_ptr, r} = from_dc.pop();
    {buf_rd_ptr, r} = from_dc.pop();
    ...
    if (error_flag)
        printf("ERROR: wrong data read from SDRAM\n");
    else
        printf("correct data read from SDRAM\n");
